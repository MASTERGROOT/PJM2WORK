...
Declare @projectId nvarchar(500) = 4
Declare @checkRemainAllocate BIT = 1
...
IF OBJECT_ID(N'tempdb..#temporg', N'U') IS NOT NULL
 ...
 Where 
( 
	(ISNULL( @projectId,'')  =''  or org.Id in (Select ncode From dbo.fn_listCode(@projectId)))
	--AND (ISNULL( @SearchprojectCode,'')  ='' or org.Code LIKE '%' + @SearchprojectCode + '%')
)	AND o.OrgCategory IN (80,85,86,87,196,197) AND o.parent != 1
...
IF OBJECT_ID('tempDB..#TempCore', 'U') IS NOT NULL
DROP TABLE #TempCore
GO
...
--######################Core######################
select a.Code,a.name,a.PMName,a.ContractAmount [(1) Contract Amount], a.BaselineBudget [(2) Baseline Budget]
		,a.CurrenctBudget [(3) Current Budget],a.ContractAmount - a.CurrenctBudget [Expected Gross Profit (1) - (3)]
		,a.Invoice [(5) Revenue],a.Receipt [(6) Receipt]
from #TempCore a
