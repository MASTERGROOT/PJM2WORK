...
FROM (
	...
	from POes p
		...
		LEFT JOIN (
			select POId, SystemCategoryId VatTypeId, SystemCategory VatType
		from POLines
		where SystemCategoryId IN (123,129,131)
		GROUP BY POId,SystemCategoryId,SystemCategory
		...
		WHERE 
			...
			and
			difcm.date <=@Todate
		--and difcm.AllocationType in (3,4)
		...
		FROM AdjustPOes ap
			...
			LEFT JOIN (
				select AdjustPOId, SystemCategoryId VatTypeId, SystemCategory VatType
			from AdjustPOLines
			where SystemCategoryId IN (123,129,131)
			GROUP BY AdjustPOId,SystemCategoryId,SystemCategory
...
FROM (
	...
	FROM SubContracts sc
		...
		LEFT JOIN Budgetlines bl ON bl.Id = ccl.BudgetLineId
		LEFT JOIN (
			SELECT SubContractId, SystemCategoryId VatTypeId, SystemCategory VatType
		FROM SubContractLines
		where SystemCategoryId IN (123,129,131)
		GROUP BY SubContractId,SystemCategoryId,SystemCategory
		...
		FROM VariationOrders vo
			...
			LEFT JOIN (
				select VariationOrderId, SystemCategoryId VatTypeId, SystemCategory VatType
			from VariationOrderLines
			where SystemCategoryId IN (123,129,131)
			GROUP BY VariationOrderId,SystemCategoryId,SystemCategory
...
FROM (
	...
	FROM Invoices i
		...
		LEFT JOIN (
			select InvoiceId, SUM(Amount) DPAmount, SUM(TaxBase) DPTaxbase, SUM(TaxAmount) DPTaxAmount
		from InvoiceLines
		where SystemCategoryId = 54
		GROUP BY InvoiceId
		) dp ON i.Id = dp.InvoiceId
		LEFT JOIN (
			select InvoiceId, SUM(Amount) RTAmount
		from InvoiceLines
		where SystemCategoryId = 48
		GROUP BY InvoiceId
		) rt ON i.Id = rt.InvoiceId
		LEFT JOIN (
			select InvoiceId, SystemCategoryId 
			VatTypeId, SystemCategory VatType
		from InvoiceLines
		where SystemCategoryId IN (123,129,131)
		GROUP BY InvoiceId,SystemCategoryId,SystemCategory
		) vat ON i.Id = vat.InvoiceId
		LEFT JOIN (
				SELECT InvoiceId, SUM(Amount) WHT
		from InvoiceLines
		where SystemCategoryId = 138
		GROUP BY InvoiceId
	...
	from Payments p
		...
		LEFT JOIN (
			SELECT Id PVLineId, ABS(ISNULL(PayAmount,0)/ NULLIF(SUM(CASE WHEN SystemCategoryId  NOT IN (53,56,57,59,74,107,111,138,141,176) THEN PayAmount END)
					OVER (PARTITION BY PaymentId),0)) Pvpt
		from PaymentLines
		where SystemCategoryId  NOT IN (53,56,57,59,74,107,111,138,141,176)
		...
		from AcctElementSets acct
			LEFT JOIN (
				SELECT InvoiceId IvId, SystemCategoryId VatTypeId, SystemCategory VatType
		...
		LEFT JOIN (
				SELECT PaymentId, SUM(PayAmount) DeductAmount
		from PaymentLines
		where SystemCategoryId = 176
		GROUP BY PaymentId
				) dd ON dd.PaymentId = p.ID
		LEFT JOIN (
				SELECT pl.PaymentId, SUM(pl.DocAmount) WHTBase, SUM(pl.PayAmount) WHT
		from PaymentLines pl
		where pl.SystemCategoryId = 138
			AND NOT EXISTS (
				SELECT 1
			FROM (
				select SendTaxThId, acs.DocId, acs.DocCode, acs.DocType, acs.docTypeId
				FROM SendTaxThLines tx
					LEFT JOIN AcctElementSets acs ON acs.id = tx.SetId
				) tax
			WHERE tax.docId = pl.PaymentId AND tax.DocTypeId = 50
				AND EXISTS (select 1
				from OtherPaymentLines opl 
							OUTER APPLY (select Id, [Date], DocStatus
					FROM OtherPayments op
					WHERE op.Id = opl.OtherPaymentId) op
				WHERE opl.RefDocId = tax.SendTaxThId AND opl.SystemCategoryId = 156 AND op.DocStatus != -1 AND op.[Date] <= @Todate
					AND EXISTS (select 1
	...
	from CommittedCostLines ccl
		...
		OUTER APPLY (
			...
			from OtherPayments op
				...
				WHERE SystemCategoryId IN (123,129,131)
				GROUP BY OtherPaymentId,SystemCategoryId,SystemCategory
					) vat ON vat.OtherPaymentId = op.Id
				LEFT JOIN ( SELECT OtherPaymentId, SUM(Amount) DeductAmount
				FROM OtherPaymentLines
				WHERE SystemCategoryId IN (176)
				GROUP BY OtherPaymentId
					) dd ON dd.OtherPaymentId = op.Id
				LEFT JOIN ( SELECT OtherPaymentId, SUM(Amount) STAmount
				FROM OtherPaymentLines
				...
				from OtherPaymentLines opl
				where opl.SystemCategoryId = 138
					AND NOT EXISTS
					(
						SELECT 1
					FROM (
					...
					WHERE tax.docId = opl.OtherPaymentId AND tax.DocTypeId = 43
						AND EXISTS (select 1
						from OtherPaymentLines opl 
														OUTER APPLY (select Id, [Date], DocStatus
							FROM OtherPayments op
				...
				GROUP BY OtherPaymentId
				) wht ON wht.OtherPaymentId = op.Id
			where opl.guid = ccl.RefDocLineGuid AND ccl.RefDocTypeId = 43
			--opl.Id = ccl.RefDocLineId AND ccl.RefDocTypeId = 43
		...
		UNION ALL
			...
			FROM workerexpenselines wel
				...
				WHERE SystemCategoryId IN (123,129,131)
				GROUP BY WorkerExpenseId,SystemCategoryId,SystemCategory
					) vat ON vat.WorkerExpenseId = wel.WorkerExpenseId
				LEFT JOIN (
				SELECT we.WorkerExpenseId, SUM(we.Amount) WHT
				FROM WorkerExpenseLines we
				WHERE we.SystemCategoryId IN (138)
					AND NOT EXISTS
					(
						SELECT 1
					FROM (
					...
					WHERE tax.docId = we.WorkerExpenseId AND tax.DocTypeId = 97
						AND EXISTS (select 1
						from OtherPaymentLines opl 
									OUTER APPLY (select Id, [Date], DocStatus
							FROM OtherPayments op
				...
				GROUP BY we.WorkerExpenseId
					) wht ON wht.WorkerExpenseId = wel.WorkerExpenseId
				LEFT JOIN (
					SELECT WorkerExpenseId, SUM(Amount) SubTotal
				FROM workerexpenselines
				WHERE (SystemCategoryId IS NULL OR SystemCategoryId NOT IN (138,123))
				GROUP BY WorkerExpenseId
					) st ON st.WorkerExpenseId = wel.WorkerExpenseId
			WHERE wel.guid = ccl.RefDocLineGuid AND ccl.RefDocTypeId = 97
		--wel.Id = ccl.RefDocLineId AND ccl.RefDocTypeId = 97
		...
		OUTER APPLY (
			...
			where opl.guid = pcl.RefDocLineGuid AND ccl.RefDocTypeId = 43
			--opl.Id = ccl.RefDocLineId AND ccl.RefDocTypeId = 43 
		...
		UNION ALL
			...
			WHERE wel.guid = pcl.RefDocLineGuid AND ccl.RefDocTypeId = 97
		--wel.Id = ccl.RefDocLineId AND ccl.RefDocTypeId = 97
...
/* มาจากเอกสาร CN ลอยที่มีการทำ PV เเละจัดสรรเข้างบที่เป็น Mat */
...
/* มาจากเอกสาร CN ลอยที่มีการทำ PV เเละจัดสรรเข้างบที่เป็น Sub */
GROUP BY LocationId,nonRef.NonRefNetPayAmount,nonRef.NonRefPayAmount,nonRef.NonRefRetentionSetAmount,nonRef.NonRefDeductAmount,nonRef.NonRefWHT
option(recompile);
/************************************************************************************************************************************************************************/
